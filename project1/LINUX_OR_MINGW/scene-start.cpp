...
// Camera locked to centre scene it seems with this crappy layout...
static float viewDist = 1.5; // Distance from the camera to the centre of the scene
static float camRotSidewaysDeg=20; // rotates the camera sideways around the centre
...
void display( void ) {
    ...
    glUniformMatrix4fv( projectionU, 1, GL_TRUE, projection );
		...
		//view gets passed via modelview matrices
		//technically, this should be inverted...
		view = RotateX(camRotation.x) * RotateY(camRotation.y) * RotateZ(camRotation.z)
					* Translate(-camPosition);
    //view = Translate(0.0, 0.0, -viewDist);
		
		//only use single point light
